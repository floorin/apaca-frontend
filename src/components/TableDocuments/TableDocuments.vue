<template>
    <div class="q-pa-xs">
        ...
        >
            ...
            <template v-if="withSearchInput" v-slot:top>
                <q-space />
                ...
                <div class="q-pa-xs col-xs-12 col-sm-6 col-md-4">
                    <q-card>
                        <q-card-section style="display: flex;">
                            <span class="text__label--thin">Nr/data doc:</span>{{ props.row.appid }}/{{ displayDataDoc(props.row.track_creation_date) }}
                            <q-icon name="folder_open" class="text-blue cursor-pointer" style="font-size: 1.2rem; margin-left: auto;" @click="openWorkOnDocument(props.row)" />
                        </q-card-section>
                        <q-separator />
                        <q-card-section class="flex" >
                            <div style="display: flex; flex-direction: column;">
                                ...
                                <div>
                                    <div v-for="repartizare in props.row.repartizari">
                                        <q-btn flat color="primary" no-caps>
                                            <div><span class="material-icons">share</span>{{repartizare.den_org_destinatar}}</div>
                                        </q-btn>
                                    </div>
                                </div>
                                <div>
        ...
        >
            ...
            <template v-slot:top v-if="withSearchInput" >
                <q-space />
                <q-input dense debounce="300" color="primary" v-model="filter">
                    <template v-slot:append>
                        <q-icon name="search" />
            ...
            <template v-slot:body="props">
                <q-tr :props="props">
                    <q-td key="appid" :props="props">
                        <q-icon name="folder_open" class="text-blue cursor-pointer" style="font-size: 1.5rem;" @click="openWorkOnDocument(props.row)" />
                    </q-td>
                    ...
                    <q-td key="repartizari" :props="props">
                        <div style="max-width:30vw;" class="container--span__file_scan">
                        <span v-for="repartizare in props.row.repartizari" class="span__repartizare">
                            <span class="material-icons">share</span>
                            <div>
                                <span style="font-size: 0.9em">{{repartizare.departament}}</span>
                                <br/>
                                <span style="font-size: 0.7em">{{repartizare.divizie}}</span>
                            </div>

                        </span>
                        </div>
                    </q-td>
                    <q-td key="titlu" :props="props">
                        <span style="font-family: 'PT Sans Narrow', sans-serif; font-size: 1em;">{{ props.row.titlu }}</span>
                    </q-td>
        ...
        <q-pagination
                ...
                v-model="currentPage"
                @input="tryToGetPageWithDocument"
...
<script lang="ts" src="./TableDocuments.ts" />
